...
Download Maven or import this directory as a Maven project into your IDE. The commands below should be run from this `spring` directory.
...
* `mvn -Pisolated spring-boot:run` to run the application with the isolated JavaScript engine.
* `mvn package` to build the Spring Boot application.
* `mvn -Pnative package` to build a GraalVM native image at `target/spring-test`.

The `isolated` profile uses `org.graalvm.polyglot:js-isolate` instead of the default JavaScript artifact and passes `-Dpolyglot.engine.SpawnIsolate=true` to the Spring Boot application and test JVMs. For Polyglot `25.1` Community Edition, use `org.graalvm.polyglot:js-isolate-community` as noted in [pom.xml](./pom.xml).
